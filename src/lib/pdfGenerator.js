...
class PDFGenerator {
      ...
      this.docDefinition = {
          ...
          content:
              [
                  ...
                  { text: 'Stamminformationen', style: 'sectionTitle', margin: [0,10,0,5] },
              ],
        styles: {
            header: {
              fontSize: 22,
  ...
  parseFormSection(section) {
      for(var e in section.properties) {
          ...
          this.docDefinition.content.push({ text: section.properties[e].title, style: 'sectionTitle', margin: [0,10,0,5]})
          for(var field in section.properties[e].properties) {
              this.parseFormField(section.properties[e].properties[field], submodel[field])
          }
      }
  }
  ...
  parseNavigation(section) {
      this.docDefinition.content.push({ text: section.title, style: 'sectionTitle', margin: [0,10,0,5]});
      this.docDefinition.content.push(                  {
                  columns: [
                    {
                        width: '30%',
